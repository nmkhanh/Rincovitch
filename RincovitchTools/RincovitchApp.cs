...
namespace RincovitchTools
{
  ...
  class RincovitchApp : IExternalApplication
  {
    static string pathCompany = @"C:\ProgramData\Autodesk\ApplicationPlugins\Rincovitch.bundle";
    static string tabName = "Rincovitch";
    ...
    //{
    //  // Lấy tên assembly cần resolve
    //  string assemblyName = new AssemblyName(args.Name).Name;

    //  // Đường dẫn đến thư mục chứa add-in
    ...
    private PushButtonGifAnimatorOptimized _animator_Print;
    private PushButtonGifAnimatorOptimized _animator_SplitFloor;
    private PushButtonGifAnimatorOptimized _animator_DrawingRegister;
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public void AddRibbonPanel(UIControlledApplication application)
    {
      ...
      try
      {
        PushButton pushButton = ribbonTool.AddItem(create_pushdata_Image(new List<string>() { "RincovitchTools.dll", "Step Automation", "RincovitchTools.Step.Rincovitch_Step", "StepAutomation.png", "A tool that automatically extracts and updates floor step (stepped slab) elevation values." })) as PushButton;
      ...
      try
      {
        PushButton pushButton = ribbonTool.AddItem(create_pushdata(new List<string>() { "RincovitchTools.dll", "Multi Print", "RincovitchTools.Print.Revit_Print", "RevitExport.png", "Print PDF and DWG" })) as PushButton;

        string gifPath = $@"{pathImage}\RevitExport.gif";
        if(File.Exists(gifPath))
          _animator_Print = new PushButtonGifAnimatorOptimized(pushButton, gifPath); // interval 120 ms
      }
      catch (Exception ex) { TaskDialog.Show("ERROR", ex.Message, TaskDialogCommonButtons.Ok); }
      ...
      try
      {
        PushButton pushButton = ribbonGeneral.AddItem(create_pushdata_Image(new List<string>() { "RincovitchTools.dll", "DrawingRegister", "RincovitchTools.DrawingRegister.Rincovitch_DrawingRegister", "DrawingRegister.png", "" })) as PushButton;

        string gifPath = $@"{pathImage}\DrawingRegister.gif";
        if (File.Exists(gifPath))
          _animator_DrawingRegister = new PushButtonGifAnimatorOptimized(pushButton, gifPath); // interval 120 ms
      }
      catch (Exception ex) { TaskDialog.Show("ERROR", ex.Message, TaskDialogCommonButtons.Ok); }

      try
      {
        PushButton pushButton = ribbonTool.AddItem(create_pushdata(new List<string>() { "RincovitchTools.dll", "Split Floor", "RincovitchTools.QuickTool.Rincovitch_SplitSlab", "RevitExport.png", "Split floor by curveloop and create model" })) as PushButton;

        string gifPath = $@"{pathImage}\SplitPlan.gif";
        if (File.Exists(gifPath))
          _animator_SplitFloor = new PushButtonGifAnimatorOptimized(pushButton, gifPath); // interval 120 ms
      }
      catch (Exception ex) { TaskDialog.Show("ERROR", ex.Message, TaskDialogCommonButtons.Ok); }
    ...
    public PushButtonData create_pushdata(List<string> infor_addin) //name_file_dll | name_in_UI | namepace.class | name_image | tooltip
    {
      ...
      try
      {
        ...
        ContextualHelp contextualHelp = new ContextualHelp(ContextualHelpType.Url, "https://rincovitch.com.au/");
    ...
    {
      _animator_Print.Dispose();
      _animator_SplitFloor.Dispose();
      _animator_Step.Dispose();
      _animator_DrawingRegister.Dispose();
      AppDomain.CurrentDomain.AssemblyResolve -= new ResolveEventHandler(CurrentDomain_AssemblyResolve);
      return Result.Succeeded;
    }
    ...
    public BitmapSource ResizeBitmap(BitmapSource source, int maxSize)
    {
      ...
      // tạo DrawingVisual để render ảnh mới
      var dv = new DrawingVisual();
      ...
      var rtb = new RenderTargetBitmap(newWidth, newHeight,
                                       source.DpiX, source.DpiY, // hoặc set cố định DPI nếu muốn
